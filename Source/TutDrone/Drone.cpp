...
ADrone::ADrone()
{
	...
	UpThruster->ThrustStrength = 980.0f; //OutCollision->GetMass() * 980.0f;
	...
	ForwardThruster->ThrustStrength = .0f;
...
void ADrone::Tick(float DeltaTime)
{
	...
	if (GetInputAxisValue(TEXT("Lift")) == .0f)
	{
		UpThruster->ThrustStrength = 980.0f;
	}

	if (GetInputAxisValue(TEXT("Forward")) == .0f)
	{
		ForwardThruster->ThrustStrength = .0f;

		if (Mesh->GetRelativeRotation().Pitch != .0f)
		{
...
void ADrone::Forward(float val)
{
	...
	{
		Mesh->AddRelativeRotation(FRotator(-val * 50.0f * GetWorld()->DeltaTimeSeconds, .0f, .0f));
	}
}
...
void ADrone::RotatePaddle(float Delta)
{
	...
	{
		paddle->AddRelativeRotation(FRotator(.0f, PaddleRotateSpeed * Delta, .0f));
	}
}

void ADrone::DoFire()
{
	FTransform firingPoint = Mesh->GetSocketTransform(TEXT("Firing"));
